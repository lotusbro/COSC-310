...
def Translate(sent):
    ...
    t= translator.translate(sent,src='en',dest='ja')
    return(f'{t.text}')
# Translator
def Tweet():
    consumer_key="zMAt7B9uLEnw0GyHryFoAVF2S"
    consumer_secret="TIsXwaRHURo7WSr0sn1gw1pKN1Rf0nh8QxjGrdBTEqHEcfITmt"
    access_key="1433506959953563669-cA85So6xc8jhz4yRb9bUn0ZFrV3cz4"
    access_secret="QHGYnPd3MX5T01HZa4BwqwLxnPCdu4rOWkUY84SduUiZw"
    auth= tweepy.OAuthHandler(consumer_key,consumer_secret)
    auth.set_access_token(access_key,access_secret)

    api=tweepy.API(auth,wait_on_rate_limit=True)
    search_words= "#spiderman"
    data= tweepy.Cursor(api.search_tweets,q=search_words,tweet_mode='extended').items(3)
...
def func(text):
    ...
    list=str(PosTag(question))
    print("User sentiment:", Sentiment(question))
    NameErrorRec(question)
    if(NameErrorRec(question)==True):
     label2.config(text=Translate(question))
     label2.config(text=Translate(response))
    #Tweet()
    print("Summary:",Wiki(question))
    # Synonym(question)
...
label2 = tk.Label(root, text='Translated if name inputs are recognized:', font=100,
                 fg='black', wraplength=500, justify='left', anchor='nw')
label2.place(relx=.55, rely=0.2, relheight=0.925, relwidth=0.97)
